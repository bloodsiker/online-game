...
class GatheringPageTest extends TestCase
{
    ...
    public function test_gathering_view_uses_server_endpoints_and_double_click_nodes(): void
    {
        ...
        $this->assertStringContainsString('class="gathering-frame gathering-frame--map"', $html);
        $this->assertStringContainsString('id="gathering-modal"', $html);
        $this->assertStringContainsString('id="gathering-modal-progress"', $html);
        $this->assertStringContainsString('id="gathering-modal-error"', $html);
        $this->assertStringContainsString('id="gathering-modal-cancel"', $html);
        $this->assertStringNotContainsString('id="gathering-modal-icon"', $html);
        $this->assertStringNotContainsString('id="gathering-modal-name"', $html);
        $this->assertStringNotContainsString('class="gathering-modal__label"', $html);
        $this->assertStringContainsString('.gathering-modal { position: absolute; top: 50%; left: 50%', $html);
        $this->assertStringNotContainsString('id="gathering-work"', $html);
        $this->assertStringContainsString('function cancelGathering()', $html);
        $this->assertStringContainsString('function showProgressModalError(message)', $html);
