...
<script>
    ...
    function ulocation(lid) {
        ...
        if (locationElement === null) {
            {{-- Если /on-map открыт без конкретного слага, возвращаем iframe на
                 карту игрока. При явном ?s= пользователь просматривает выбранную
                 карту, поэтому отсутствие на ней текущей позиции — нормально. --}}
            @if(request()->routeIs('on_map') && ! request()->filled('s'))
