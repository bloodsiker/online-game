...
@if ($page->activeSession)
    <div class="active-run-banner">
        <b>Вы в данже:</b> {{ $page->activeSession->dungeonName }}
...
@forelse ($page->dungeons as $dungeon)
    <div class="dungeon-item">
        ...
        <div class="dungeon-meta">
            Игроков: {{ $dungeon->maxPlayers === 1 ? 'Соло' : 'до ' . $dungeon->maxPlayers }} &bull;
            Мин. уровень: {{ $dungeon->minLevel }}
            @if ($dungeon->cooldownSeconds > 0)
                &bull; КД: {{ round($dungeon->cooldownSeconds / 3600, 1) }} ч.
            @endif
            @if ($dungeon->timeLimitSeconds)
                &bull; Таймер: {{ round($dungeon->timeLimitSeconds / 60) }} мин.
            @endif
            @if ($dungeon->requiresKey)
                &bull; Ключ: {{ $dungeon->entryKeyName ?? '?' }}
            @endif
            @if ($dungeon->entryLocationId)
                &bull; Вход: лок. #{{ $dungeon->entryLocationId }}
            @endif
        </div>
