...
<head>
    ...
    <style>
        * { font-size: 11px; }
        ...
        .itm { width: 50px; height: 50px; border: 0; vertical-align: middle; border-right: 1px solid #CEBBAA; margin-right: 8px; }
        ...
        .footer-row td { padding: 4px 8px; color: #7a5030; }
    </style>
</head>
<body>
                            ...
                            <tr class="t1">
                                <td colspan="3" style="padding: 4px 10px;">
                                    <b style="color:#461c0b;">
                                        @if($page->count)
                                            Предметы на локации ({{ $page->count }})
                                        @else
                                            Предметы на локации
                                        @endif
                                    </b>
                                ...
                                <tr class="{{ $i % 2 === 0 ? 'l0' : 'l1' }}">
                                    ...
                                    <td style="padding: 3px 8px;" align="right">
                                        <b class="butt2 pointer"><b>
                                            <input value="{{ $item->actionLabel }}" type="button" onclick="location.href='{{ $item->actionUrl }}'">
                                        </b></b>
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="3" class="empty-msg">Вы не нашли здесь ничего стоящего...</td>
                                </tr>
                            @endforelse
                            <tr class="footer-row t1">
                                <td colspan="3">
                                    <b class="butt2 pointer"><b>
                                        <input value="« Описание местности" type="button" onclick="window.parent.sendDataToGame('{{ $page->backUrl }}')">
                                    </b></b>
                                </td>
                            </tr>
                            </tbody>
