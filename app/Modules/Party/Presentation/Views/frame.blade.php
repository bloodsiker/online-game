                    ...
                    <div class="member-row">
                        ...
                        @if($isLeader && !$mIsLeader)
                            &nbsp;<form method="POST" action="{{ route('party.kick', ['partyId' => $party->id]) }}" style="display:inline;">
                                ...
                                <a href="#" class="kick-link" title="Выгнать из группы" data-player-name="{{ $mUser->name }}" onclick="return confirmPartyKick(event, this);">[выгнать]</a>
                            </form>
                        @endif
                    </div>
                @endforeach
                ...
                <div align="center" style="margin-top: 10px;">
                    ...
                    @else
                        <form method="POST" action="{{ route('party.leave', ['partyId' => $party->id]) }}">
                            @csrf
                            <span class="butt1 pointer">
                                <span><input value="Покинуть группу" type="button" class="grnn" onclick="return confirmPartyLeave(event, this);"></span>
                            </span>
                        </form>
                    @endif
                </div>
...
<script>
    (function showPartyActionMessage() {
        var message = @json(session('party_success') ?? session('party_error'));
        if (!message) return;

        var title = @json(session('party_error') ? 'Ошибка группы' : 'Группа');
        var shouldRefreshChat = @json(session('party_success') !== null);
        var attemptsLeft = 20;
        var showMessage = function () {
            try {
                if (typeof window.top.openGameMessageModal === 'function') {
                    window.top.openGameMessageModal({ title: title, message: message });
                    if (shouldRefreshChat && typeof window.top.refreshChatChannels === 'function') {
                        window.top.refreshChatChannels();
                    }
            ...
            attemptsLeft -= 1;
            if (attemptsLeft > 0) setTimeout(showMessage, 50);
        };
    ...
    function confirmPartyLeave(event, trigger) {
        if (event) {
        ...
        try {
            if (typeof window.top.openGameConfirmModal !== 'function') return false;

            window.top.openGameConfirmModal({
    ...
    function confirmPartyKick(event, trigger) {
        if (event) {
            event.preventDefault();
            event.stopPropagation();
        }
        ...
        try {
            ...
            window.top.openGameConfirmModal({
                ...
                message: 'Вы действительно хотите исключить игрока ' + (trigger.dataset.playerName || '') + ' из группы?',
                onConfirm: function () { form.submit(); },
            });
        } catch (e) {}
