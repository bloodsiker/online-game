...
<td width="55%">
    ...
    <div class="ref-link-box">
        <input type="text" readonly value="{{ $page->referralLink }}" onclick="this.select()" id="ref-link">
    </div>
    ...
    <div style="padding: 4px 10px 8px; color: #555;">
        Вас пригласил: <b>{{ $page->invitedByName }}</b>
    </div>
    @endif

    <div class="section-title" style="margin-top:6px">Приглашённые игроки ({{ count($page->referred) }})</div>
        ...
        @foreach($page->referred as $referral)
            <tr class="{{ $loop->even ? 'bg_l' : '' }}">
                <td class="p10h p4v">
                    ...
                    @else
                        <span class="user_offline">{{ $referral->name }}</span>
                    @endif
                </td>
                <td class="p4v" align="center">{{ $referral->level ?? '—' }}</td>
                ...
                <td class="p4v user_offline" align="center">
                    {{ $referral->lastOnlineLabel ?? '—' }}
                </td>
                <td class="p4v" align="center">{{ $referral->claimedRewardsCount }} / {{ $referral->totalStagesCount }}</td>
            </tr>
        @endforeach
        </tbody>
    </table>
