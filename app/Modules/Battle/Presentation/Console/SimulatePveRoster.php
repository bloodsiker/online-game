...
class SimulatePveRoster extends Command
{
    ...
    private function meetsRequirements(ShareItem $item, int $level, array $stats): bool
    {
        foreach ($item->requirements as $requirement) {
            if ($requirement->type === ShareItemRequirementType::LEVEL && $level < $requirement->min_value) {
                return false;
            }
            if ($requirement->type === ShareItemRequirementType::STAT && ($stats[$requirement->stat_key] ?? 0) < $requirement->min_value) {
                return false;
            }
            if ($requirement->type === ShareItemRequirementType::SKILL) {
                return false;
            }
        }
    ...
    private function scoreItem(ShareItem $item, bool $mage, array $stats): int
    {
        ...
        foreach ($item->stats as $stat) {
            $score += match ($stat->stat_type) {
                ShareItemStatType::ATTACK_MIN, ShareItemStatType::ATTACK_MAX => $mage ? 0 : (int) $stat->value * 6, ShareItemStatType::INTELLIGENCE, ShareItemStatType::WISDOM, ShareItemStatType::MAGIC_RESISTANCE => $mage ? (int) $stat->value * 5 : (int) $stat->value, default => (int) $stat->value * 3
            };
        }
...
final class RosterReferenceFighter implements FightHitInterface
{
    ...
    public function __construct(private int $level, private array $stats, private array $bonus, private bool $mage)
    {
        ...
        $this->minDamage = max(1, (int) floor((1 + $bonus['attack_min']) * $scale));
        $this->maxDamage = max($this->minDamage, (int) floor((2 + $bonus['attack_max']) * $scale));
    }
    ...
    {
        return max(0, ($this->stats['strength'] - 1) + $this->bonus['armor']);
    }

    public function getDodge(): int
    {
        return max(0, ($this->stats['agility'] - 1) + $this->bonus['dodge']);
    }

    public function getCritical(): int
    {
        return max(0, ($this->stats['intuition'] - 1) + $this->bonus['critical']);
    }

    public function getIntelligence(): int
    {
        return $this->stats['intelligence'] + $this->bonus['intelligence'];
    }

    public function getMagicResistance(): int
    {
        return max(0, $this->stats['wisdom'] - 1 + $this->bonus['magic_resistance']);
    }
    ...
    {
        $total = max(1, $this->stats['strength'] + $this->stats['agility'] + $this->stats['intuition']);

        return match ($class) {
            CombatClass::TANK => $this->stats['strength'] / $total, CombatClass::DODGE => $this->stats['agility'] / $total, CombatClass::CRIT => $this->stats['intuition'] / $total
        };
    }
